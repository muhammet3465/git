...
<body>
    ...
    if (@$_GET["is"] == "aktif") {
        ...
        $aktif = $db->prepare("UPDATE `blog1` SET `aktif` = :a WHERE `id` = :i");
    ...
    if(@$_GET["is"] == "sil"){
        $sil = $db->prepare("DELETE FROM `blog1` WHERE `id` = :i");
                        ...
                        <div class="panel-body">
                            <div class="dataTable_wrapper">
                                <table width="100%" class="table table-striped table-bordered table-hover"
                                    ...
                                    <tbody>
                                        <?php
                                        $cek = $db->prepare("SELECT * FROM  `blog1` ORDER BY `id` DESC");
                                        $cek->execute();
                                        ...
                                        ?>
                                            ...
                                            ('Aktiflik durumu değişsin mi ?')" class="btn btn-success btn-xs"
                                                            style="margin-right: 15px">Aktif</a>
                                                    <?php } else { ?>
                                                        <a href="blog.php?is=aktif&id=<?= $row["id"] ?>&drm=<?= $row["aktif"] ?>"
                                                            onclick="return confirm
                                            ('Aktiflik durumu değişsin mi ?')" class="btn btn-danger btn-xs"
